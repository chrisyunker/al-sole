...
<body>
    <div class="container">
        <h1><img src="favicon.png" alt="Sun icon">Al Sole</h1>
        <h3><center>(To the Sun)</center></h3>
        ...
        <div id="mainContent" style="display: none;">
            ...
            <button class="info-button" onclick="openModal()" style="margin-top: 5px;">About</button>
        </div>
    </div>
    ...
    <div id="formulaModal" class="modal" onclick="closeModalOnOutsideClick(event)">
        <div class="modal-content">
            ...
            <p style="text-align: left;">
                Website by <a href="https://yunker.dev" target="_blank" style="color: #60a5fa; text-decoration: none;">Chris Yunker</a><br>
            </p>

            <h3 style="margin-top: 30px; text-align: center; font-size: 1.6em;">Formulas</h3>
            ...
            <h3>5. Solar Elevation Angle</h3>
            <p>The angle of the Sun above or below the horizon at your location:</p>

            <div class="formula">
                <div class="formula-title">Solar Declination:</div>
                <span class="variable">δ</span> = 23.45° × sin(360° × (284 + <span class="variable">N</span>) / 365)
            </div>

            <p><strong>Where:</strong></p>
            <p>
                • <span class="variable">δ</span> = solar declination (Sun's angle relative to equator)<br>
                • <span class="variable">N</span> = day of year (1-365)<br>
                • Varies from -23.45° (winter solstice) to +23.45° (summer solstice)
            </p>

            <div class="formula">
                <div class="formula-title">Solar Elevation Angle:</div>
                sin(<span class="variable">α</span>) = sin(<span class="variable">φ</span>) × sin(<span class="variable">δ</span>) + cos(<span class="variable">φ</span>) × cos(<span class="variable">δ</span>) × cos(<span class="variable">H</span>)
            </div>

            <p><strong>Where:</strong></p>
            <p>
                • <span class="variable">α</span> = solar elevation angle<br>
                • <span class="variable">φ</span> = observer's latitude<br>
                • <span class="variable">δ</span> = solar declination<br>
                • <span class="variable">H</span> = solar hour angle<br>
                • Positive values: Sun above horizon (daytime)<br>
                • Negative values: Sun below horizon (nighttime)<br>
    ...
    <script>
        ...
        const colorPalette = ['#60a5fa', '#ff6b6b', '#4ecdc4', '#ffd93d', '#a78bfa'];
        ...
        function drawGraph() {
            ...
            // Draw left Y-axis labels (Distance as offset from average)
            ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
            ctx.font = '12px sans-serif';
            ctx.textAlign = 'right';

            // Calculate average distance for baseline
            const avgDistance = (minDistance + maxDistance) / 2;
            const unitSuffix = currentUnit === 'miles' ? ' mi' : ' km';

            for (let i = 0; i <= 5; i++) {
                const y = paddingTop + (graphHeight * i / 5);
                const value = maxDistance - (distanceRange * i / 5);
                const offset = value - avgDistance;

                // Format offset with + or - sign
                const sign = offset >= 0 ? '+' : '';
                const label = sign + offset.toFixed(0) + unitSuffix;
                ctx.fillText(label, paddingLeft - 10, y + 4);
            }

            // Draw baseline indicator label
            ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
        ...
        function requestLocation() {
            ...
            if ('geolocation' in navigator) {
                // Set a timeout to fallback to default location if geolocation takes too long
                const timeoutId = setTimeout(() => {
                    statusEl.className = 'status error';
                    statusEl.innerHTML = 'Location access timed out. Using default location (Boston, MA).<br><small>For local file testing, try using a local web server.</small>';
                    // Use default location (Boston, MA)
                    setTimeout(() => {
                        onLocationSuccess({ coords: { latitude: 42.3601, longitude: -71.0589 } });
                    }, 2000);
        ...
        function onLocationError(error) {
            ...
            switch(error.code) {
                case error.PERMISSION_DENIED:
                    errorMessage = 'Location access denied. Using default location (Boston, MA).';
                    break;
                case error.POSITION_UNAVAILABLE:
                    errorMessage = 'Location information unavailable. Using default location (Boston, MA).';
                    break;
                case error.TIMEOUT:
                    errorMessage = 'Location request timed out. Using default location (Boston, MA).';
                    break;
                default:
                    errorMessage = 'An unknown error occurred. Using default location (Boston, MA).';
            }

            statusEl.innerHTML = errorMessage + '<br><small>For local file testing, try using a local web server.</small>';

            // Use default location (Boston, MA) after showing error for 2 seconds
            setTimeout(() => {
                onLocationSuccess({ coords: { latitude: 42.3601, longitude: -71.0589 } });
            }, 2000);
        }
